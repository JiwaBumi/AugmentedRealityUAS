...
@Composable
fun GrasslampProduct(navController: NavController, cartViewModel: CartViewModel) {
    val productId = 5
    ...
    val recommendedProducts = productList.filter { it.first != "grasslamp" }.shuffled().take(3)
    ...
    Scaffold(
        ...
        Column(
            ...
            Row(
                ...
                // Placeholder Image
                Image(
                    painter = painterResource(id = R.drawable.mo_grasslamp),
                ...
                // Product Info
                Column {
                    Text("Grass Lamp", style = MaterialTheme.typography.headlineSmall)
                    Text("Bring nature indoors with this eco-inspired lamp! \nRp 700.000", style = MaterialTheme.typography.bodyMedium)
            ...
            Text(
                text = """
                    This unique lamp combines a nature-inspired design with eco-friendly materials, ideal for adding a touch of serenity and warmth to any room. Its gentle lighting creates a peaceful atmosphere, perfect for nature lovers!
                """.trimIndent(),
...
@Composable
fun GrasslampProductPreview() {
    GrasslampProduct(navController = rememberNavController(), cartViewModel = viewModel())
}
